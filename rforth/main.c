...
void execute_commands_from_string(const char* commands, int_stack_t* stack);
...
void execute_command(const char* command, int_stack_t *stack) {
    if (isdigit(command[0]) || (command[0] == '-' && isdigit(command[1]))) {
        int_stack_push(stack, atoi(command));
    } else {
        const char* dict_command = get_command_from_dictionary(command);
        if (dict_command) {
            execute_commands_from_string(dict_command, stack);
        } else {
...
int main() {
    ...
    printf("Enter your SN-FORTH code ('end' to stop):\n");
